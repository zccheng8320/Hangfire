...
namespace Hangfire.Dashboard.Management.Support
{
    ...
    {
        public static string GetParams(Common.Job job)
        {
            var paramInfos = job.Method.GetParameters();
            var jobParams = job.Args;
            var list = new List<string>();
            for (int i = 0; i < paramInfos.Length; i++)
            {
                var parameterInfo = paramInfos[i];
                if (parameterInfo.ParameterType == typeof(IJobCancellationToken) || parameterInfo.ParameterType == typeof(PerformContext))
                {
                    continue;
                }

                var obj = jobParams[i];
                var valueString = obj?.ToString() ??"null";
                if (obj is DateTime d)
                {
                    ...
                    {
                        valueString = d.ToString("f");
                    }
                }

                var value = $"{parameterInfo.Name}:{valueString}";
                list.Add(value);
            }
